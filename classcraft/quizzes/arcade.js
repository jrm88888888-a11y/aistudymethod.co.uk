...
/* ── fitLayout ──────────────────────────────────────────────────────────────
   Reorganises an already-rendered game into the .ar-fit cockpit layout so the
   board and the question sit side by side and the page never scrolls.

   Games build their markup from nested template literals, so rewriting those
   templates by hand is error-prone. This instead wraps the DOM *after* render:
   everything above the board becomes the head, the board's top-level ancestor
   becomes the board, and everything from the question onward becomes the panel.

   Idempotent, and re-applies itself when the game re-renders (most games blow
   away root.innerHTML on every question). Call it once, early - the observer
   picks the layout up as soon as the selectors first appear.

     Arcade.fitLayout(root, { board: '.gr-rail', panel: '.gr-q' });
*/
Arcade.fitLayout = function (root, opts) {
  if (!root || !opts || !opts.board || !opts.panel) return;
  var busy = false, obs = null;

  function host() {
    return opts.within ? (root.querySelector(opts.within) || root) : root;
  }
  function topLevel(el, h) {
    while (el && el.parentNode !== h) el = el.parentNode;
    return el;
  }
  ...
  function apply() {
    if (busy) return;
    var h = host();
    if (h.querySelector(':scope > .ar-fit-board')) return;      // already wrapped
    var board = h.querySelector(opts.board);
    var panel = h.querySelector(opts.panel);
    if (!board || !panel) return;                                // not rendered yet
    var bTop = topLevel(board, h), pTop = topLevel(panel, h);
    if (!bTop || !pTop || bTop === pTop) return;                 // no clean split
    var kids = Array.prototype.slice.call(h.children);
    var bi = kids.indexOf(bTop), pi = kids.indexOf(pTop);
    if (bi < 0 || pi < 0 || pi <= bi) return;

    busy = true;
    if (obs) obs.disconnect();
    try {
      var head = document.createElement('div'); head.className = 'ar-fit-head';
      var bw   = document.createElement('div'); bw.className   = 'ar-fit-board';
      var pw   = document.createElement('div'); pw.className   = 'ar-fit-panel';
      // Anything between the board and the question (HUD, progress dots) is
      // status, not gameplay - it belongs in the header strip, not squashed
      // into the board cell or padding out the answer panel.
      kids.forEach(function (c, i) {
        (i === bi ? bw : i < pi ? head : pw).appendChild(c);
      });
      h.appendChild(head); h.appendChild(bw); h.appendChild(pw);
      h.classList.add('ar-fit-grid');
      root.classList.add('ar-fit');
    } catch (e) { /* layout is a nicety - never break the game */ }
    busy = false;
    if (obs) obs.observe(root, { childList: true, subtree: false });
  }

  apply();
  obs = new MutationObserver(function () { apply(); });
  obs.observe(root, { childList: true, subtree: false });
  return { refresh: apply, stop: function () { obs && obs.disconnect(); } };
};
