/* Engine shell — cloned from adventures/biology-gcse-aqa-cell-biology-mini-lesson.html
   Only visible content + game data vary. Generic ids: sortChips / binK* / mLeft / mRight. */
...
const CSS = `  :root{
  ...
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  .tag.core{background:#e7fbef;color:#137a45}
  .tag.maths{background:#efe1ff;color:#6a30b8}
  ...
  .reveal-btn{font-family:'Fredoka';font-weight:600;font-size:.95rem;background:#fff;border:2.5px solid var(--ink);border-radius:12px;padding:7px 14px;cursor:pointer;box-shadow:0 3px 0 var(--ink)}
  .reveal-body{display:none;margin-top:10px}
  .reveal-body.show{display:block}
  ...
  .bin.b0{background:#eef6ff}.bin.b1{background:#eafaf0}.bin.b2{background:#fdeef0}
...
const FILL = ['#d9ebff', '#d6f6e4', '#efe1ff', '#ffe9c7'];
const STROKE = ['#0b5bbf', '#137a45', '#6a30b8', '#9a5a00'];

function pillarSvg(pillars, caption) {
  // pillars: array of 2-4 [line1, line2] label pairs
  const n = pillars.length;
  const gap = 12, pad = 14, W = 560;
  const w = Math.floor((W - 2 * pad - gap * (n - 1)) / n);
  let s = `<svg viewBox="0 0 560 150" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="${caption}">`;
  pillars.forEach((p, i) => {
    const x = pad + i * (w + gap), cx = x + w / 2;
    s += `<rect x="${x}" y="42" width="${w}" height="64" rx="14" fill="${FILL[i % 4]}" stroke="#2c2840" stroke-width="3"/>`;
    s += `<text x="${cx}" y="${p[1] ? 70 : 80}" text-anchor="middle" font-family="Fredoka" font-size="13" font-weight="600" fill="${STROKE[i % 4]}">${p[0]}</text>`;
    if (p[1]) s += `<text x="${cx}" y="90" text-anchor="middle" font-family="Fredoka" font-size="13" font-weight="600" fill="${STROKE[i % 4]}">${p[1]}</text>`;
  });
  s += `<text x="280" y="132" text-anchor="middle" font-family="Nunito" font-size="12.5" fill="#6b6580">${caption}</text></svg>`;
  return s;
}
...
function render(L) {
  ...
  // 0 WELCOME  (the ONLY screen with class="screen active")
  body.push(`    <!-- 0 WELCOME -->
    <section class="screen active">
      ...
      <figure>
        ${pillarSvg(L.pillars, L.pillarCaption)}
      </figure>
      <p>Work through each screen, answer the <b>questions</b> as you go (several are full A-level calculations) and collect ⭐ stars. Everything here is written to the <b>Edexcel 9CH0</b> content statements for <b>${L.specTopic}</b>. Press <b>Start</b> when you are ready.</p>
    </section>`);

  L.screens.forEach((s, idx) => {
    if (s.t === 'teach') {
      body.push(`    <!-- ${idx + 1} TEACH -->
...
${opts}
      ...
      ni++;
      const key = 'n' + ni;
      body.push(`    <!-- ${idx + 1} NUMERIC ${key} -->
    <section class="screen">
      <span class="tag maths">Calculate</span>
      <h2>${s.h}</h2>
      <div class="activity">
        <div class="qline"><span class="qnum">${ni}</span><span>${s.q}</span></div>
        <div class="numwrap" data-num="${key}" data-answer="${s.ans}" data-tol="${s.tol}">
          <input type="text" inputmode="text" aria-label="Your answer"><span class="unit">${s.unit}</span>
      ...
      const g = L.sort;
      const bins = g.bins.map((b, i) => `          <div class="bin b${i}" data-bin="${b.key}"><h3>${b.label}</h3><div class="drop" id="binK${i}"></div></div>`).join('\n');
      body.push(`    <!-- ${idx + 1} SORT GAME -->
    <section class="screen">
      <span class="tag">Sort it</span>
      <h2>${g.h}</h2>
      <p style="font-size:1.02rem;color:var(--soft)">${g.prompt}</p>
      <div class="activity">
        <div class="chips" id="sortChips"></div>
        <div class="bins" style="grid-template-columns:repeat(${g.bins.length},1fr)">
${bins}
        </div>
      ...
      const g = L.match;
      body.push(`    <!-- ${idx + 1} MATCH GAME -->
    <section class="screen">
      <span class="tag">Match up</span>
      <h2>${g.h}</h2>
      <p style="font-size:1.02rem;color:var(--soft)">${g.prompt}</p>
      <div class="activity">
        <div class="match">
          <div class="mcol"><div class="mhead">${g.leftHead}</div><div class="mcol" id="mLeft"></div></div>
          <div class="mcol"><div class="mhead">${g.rightHead}</div><div class="mcol" id="mRight"></div></div>
        </div>
  ...
  body.push(`    <!-- RECAP -->
    <section class="screen">
      ...
      <h2>The big ideas to take away</h2>
      <div class="recap">
${L.recap.map(r => `        <p><b>${r[0]}:</b> ${r[1]}</p>`).join('\n')}
      </div>
      <p>That is <b>${L.specTopic}</b> of Edexcel A-level Chemistry (9CH0). Press <b>Finish</b> to see your score.</p>
    </section>`);
  ...
  body.push(`    <!-- FINAL -->
    <section class="screen final">
      ...
      <p class="big">You have worked through <b>${L.title}</b> for Edexcel A-level Chemistry. 🎉</p>
  ...
  const sortData = JSON.stringify(L.sort.chips);
  const matchPairs = JSON.stringify(L.match.pairs.map(p => ({ l: p[0], r: p[1] })));
...
<title>${L.title} — A-level Chemistry Mini-Lesson · AI Study Method</title>
...
<div class="app">
  ...
  <div class="spec">Edexcel A-level Chemistry (9CH0) · ${L.specTopic}</div>
...
document.querySelectorAll('.opts[data-q]').forEach(b=>setupMCQ(b.dataset.q));

function setupNum(nkey){const box=document.querySelector('.numwrap[data-num="'+nkey+'"]');if(!box)return;const ans=parseFloat(box.dataset.answer);const tol=parseFloat(box.dataset.tol||'0.5');const input=box.querySelector('input');const btn=box.querySelector('.checkbtn');const fb=document.getElementById('fb-'+nkey);let tries=0;function check(){if(done[nkey])return;const raw=(input.value||'').replace(/[, ]/g,'').replace(/[^0-9.+\\-eE]/g,'').trim();if(raw===''){fb.textContent='Type your answer first ✍️';fb.className='feedback';return;}const v=parseFloat(raw);if(!isNaN(v)&&Math.abs(v-ans)<=tol){input.classList.add('correct');input.disabled=true;btn.disabled=true;fb.textContent='🎉 Correct! +1 star';fb.className='feedback good';addScore(1);done[nkey]=true;refreshGate();}else{tries++;input.classList.add('wrong');fb.textContent=tries>=2?('Answer: '+ans+'. Check your working, then move on.'):'Not quite — recalculate and try again 💪';fb.className='feedback bad';if(tries>=2){done[nkey]=true;input.value=ans;input.disabled=true;btn.disabled=true;refreshGate();}else{setTimeout(()=>{input.classList.remove('wrong');input.focus();input.select();},500);}}}btn.addEventListener('click',check);input.addEventListener('keydown',e=>{if(e.key==='Enter')check();});}
document.querySelectorAll('.numwrap[data-num]').forEach(b=>setupNum(b.dataset.num));

// sort game — [label, binKey]
const sortData=${sortData};
(function(){const chips=document.getElementById('sortChips');if(!chips)return;const fb=document.getElementById('fb-sort');let sel=null,placed=0;
sortData.forEach((r,i)=>{const c=document.createElement('button');c.className='chip';c.textContent=r[0];c.dataset.k=r[1];c.dataset.i=i;c.addEventListener('click',()=>{chips.querySelectorAll('.chip').forEach(x=>x.classList.remove('sel'));c.classList.add('sel');sel=c;});chips.appendChild(c);});
function place(binKey,binEl){if(!sel){fb.textContent='Tap a card first ⬆';fb.className='feedback';return;}if(sel.dataset.k===binKey){const tag=document.createElement('span');tag.className='placed ok';tag.textContent=sel.textContent;binEl.appendChild(tag);sel.remove();sel=null;placed++;addScore(1);fb.textContent='✅ Correct!';fb.className='feedback good';if(placed===sortData.length){fb.textContent='🌟 ${L.sort.doneMsg}';fb.className='feedback good';done.sort=true;refreshGate();}}else{fb.textContent='Not that group — think again 🤔';fb.className='feedback bad';sel.classList.add('miss');setTimeout(()=>sel&&sel.classList.remove('miss'),400);}}
document.querySelectorAll('.bin').forEach(b=>b.addEventListener('click',()=>place(b.dataset.bin,b.querySelector('.drop'))));})();

// match game
const matchPairs=${matchPairs};
(function(){const L=document.getElementById('mLeft'),R=document.getElementById('mRight');if(!L)return;const fb=document.getElementById('fb-match');let selL=null,matched=0;const rOrder=matchPairs.map((_,i)=>i).sort(()=>Math.random()-0.5);
matchPairs.forEach((p,i)=>{const a=document.createElement('div');a.className='mitem';a.textContent=p.l;a.dataset.i=i;a.addEventListener('click',()=>{if(a.classList.contains('done'))return;L.querySelectorAll('.mitem').forEach(x=>x.classList.remove('sel'));a.classList.add('sel');selL=a;});L.appendChild(a);});
rOrder.forEach(i=>{const p=matchPairs[i];const b=document.createElement('div');b.className='mitem';b.textContent=p.r;b.dataset.i=i;b.addEventListener('click',()=>{if(b.classList.contains('done'))return;if(!selL){fb.textContent='Pick a card on the left first ⬅';fb.className='feedback';return;}const li=selL.dataset.i;const want=matchPairs[li].r;if(p.r===want){selL.classList.add('done');selL.classList.remove('sel');b.classList.add('done');addScore(1);matched++;selL=null;if(matched===matchPairs.length){fb.textContent='🌟 ${L.match.doneMsg}';fb.className='feedback good';done.match=true;refreshGate();}else{fb.textContent='✅ Matched!';fb.className='feedback good';}}else{b.classList.add('miss');setTimeout(()=>b.classList.remove('miss'),400);fb.textContent='Not that one — re-read the card 🤔';fb.className='feedback bad';}});R.appendChild(b);});})();
...
(function(){
  ...
  function share(kind){
    ...
    var txt="I scored "+sc+"/"+mx+" ⭐ on the "+mm.topic+" mini-lesson — bet you can't beat me! Try it: https://aistudymethod.com";
